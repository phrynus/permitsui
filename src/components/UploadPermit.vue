...
const cityRegexList = ref<[string, RegExp][]>([
  ...
  ['北京', /(?:北京)[省市]?/],
  ['重庆', /(?:重庆)[省市]?/],
  ['福建', /(?:福安|福建|福州|厦门|莆田|三明| 泉州|漳州|南平|龙岩|宁德)[省市]?/],
  ...
  ['湖南', /(?:湖南|长沙|株洲|湘潭|衡阳|邵阳|岳阳|常德|张家界|益阳|郴州|永州|怀化|娄底|龙山)[省市县]?/],
...
提取公司名称、发证时间、许可证编号
判断业务是否包含：ICP、EDI、IDC、CDN、ISP、VPN、SP、多方通信、呼叫中心、文网文、出版物、广播、变更
判断类型是否是：许可证、受理通知书、批准通知书
判断省份地区：可根据公司名称，和地区短标识判断地区，然后取地区省份
判断发证时间:注意区分提交时间和发证时间,发证时间一般在最后日期,格式yyyy-dd-mm,如果没有就写今天
